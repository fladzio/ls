...
void get_permissions(file_details *file, struct stat *statbuf)
{
	...
	if (S_ISREG(statbuf->st_mode))
		file->perms[0] = '-'; // regular
	if (S_ISLNK(statbuf->st_mode))
		file->perms[0] = 'l'; // link
	if (S_ISDIR(statbuf->st_mode))
		file->perms[0] = 'd'; // directory
...
void list_dir(char *program, char *name, int details, int recursively, int time_sort, int human)
{
	...
	while ((dp = readdir(dir)) != NULL)
	{
		...
		strcpy(filelist[++k].name, dp->d_name);
		filelist[k].is_directory = S_ISDIR(statbuf.st_mode) ? 1 : 0;
		get_permissions(&filelist[k], &statbuf);
		filelist[k].nlink = statbuf.st_nlink;
		filelist[k].uid_name = statbuf.st_uid;
		filelist[k].gid_name = statbuf.st_gid;
		filelist[k].size = statbuf.st_size;
	...
	// lista folderow w obecnym katalogu
	file_details *dir_list = (file_details *)malloc(sizeof(file_details) * dir_count);
	...
	for (int i = 0; i < file_opened; ++i)
	{
		if (filelist[i].is_directory)
		{
			if (strcmp(filelist[i].name, ".") == 0 || strcmp(filelist[i].name, "..") == 0)
				continue;
			dir_list[dir_iteration++] = filelist[i];
		}
	}
